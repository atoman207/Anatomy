...
try {
  ...
  await page.click("text=貼り付けた名前を追加");
  await page.waitForSelector("text=ファイル一覧");
  await shot("organize-inventory");

  // Scope to the inventory card; the page also renders a group-summary table.
  const inventoryCard = page.locator("section").filter({ hasText: "ファイル一覧" }).last();
  ...
  await page.waitForSelector("text=このシートを使う");
  await shot("organize-samplesheet");
  const groupsText = await page.locator("text=群の構成").count();
  if (groupsText === 0) problems.push("Group composition card missing");

  // rename tab
  await page.click('button[role="tab"]:has-text("ファイル名変更")');
  await page.waitForSelector("text=変更ルール");
  await shot("organize-rename");

  // ---------- analyze: demo data ----------
  console.log("Analyze");
  await go("/analyze");
  await page.click("text=デモデータを読み込む");
  await page.waitForSelector("text=前処理", { timeout: 30000 });
  await shot("analyze-import");

  const featureTile = await page.locator("text=特徴量").first().isVisible();
  if (!featureTile) problems.push("Feature count tile missing after demo load");

  // statistics
  await page.click('button[role="tab"]:has-text("統計解析")');
  await page.waitForSelector("text=解析手法", { timeout: 30000 });
  await page.waitForSelector("text=差次発現", { timeout: 30000 });
  await shot("analyze-differential");

  const upTile = await page.locator("text=上昇").first().isVisible();
  if (!upTile) problems.push("Differential result tiles missing");

  // ANOVA
  await page.click('button:has-text("ANOVA")');
  await page.waitForSelector("text=分散分析表", { timeout: 60000 });
  await shot("analyze-anova");

  // PCA
  await page.click('button:has-text("PCA")');
  await page.waitForSelector("text=寄与率", { timeout: 60000 });
  ...
  await page.waitForSelector("text=全特徴量", { timeout: 60000 });
  ...
  await page.click('button:has-text("ヒートマップ")');
  ...
  await page.click('button:has-text("PCAプロット")');
  await page.waitForTimeout(900);
  await shot("figure-pca");

  // send a figure to the notebook
  await page.click("text=ノートへ");

  // ---------- notebook ----------
  console.log("Notebook");
  await go("/notebook");
  await page.waitForSelector("text=プレビュー");
